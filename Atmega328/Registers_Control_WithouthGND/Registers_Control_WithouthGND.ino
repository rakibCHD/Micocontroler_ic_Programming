//Going to glow led & blinking with Registers(without Ground Connection).
...
//---------------Power:Max_5v,Min_1.8v[(P+)-->ChipPin:7+20][(N-)-->ChipPin:8+22]-------------|
...
void setup() {
    //DDRD = 0x0C;       // Configure pins 2 and 3 as output.
    
    DDRD = (1 << PD2) | (1 << PD3);//// Configure pins PD2 and PD3 as output.
    
    //PORTD &= ~0x08;    // Ensure PD3 is LOW (clear bit 3).
    
    PORTD &= ~(1 << PD3); // Ensure pin PD3 is LOW.
}
void loop() {
  
  //PORTD = 0x04;     // Set pin 2 HIGH.

  PORTD = 0b00000100;// Set pin 2 HIGH.
  
  //PORTD |= (1 << PD2); // Set pin PD2 HIGH.
  
  sleep(2500);
  
  //PORTD = 0x00;      // Set all pin on PortD is LOW.
  
  PORTD &= ~0x04;  // Set pin 2 LOW (clear bit 2).

  //PORTD &= ~0b00000100; // Set pin 2 LOW (clear bit 2).
